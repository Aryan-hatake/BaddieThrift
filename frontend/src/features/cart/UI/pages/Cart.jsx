...
const QuantityStepper = ({ quantity, onDecrease, onIncrease }) => (
  <div className="flex items-center border-2 border-black h-12 bg-white">
    ...
    <span className="w-12 text-center font-black font-['Space_Grotesk'] text-sm select-none">
...
const Cart = () => {
  ...
  const { cartItems, loading, error, cartPrice, cartId, cartCurrency } =
    useSelector((state) => state.cart);

  const user = useSelector((state) => state.auth?.user);
  const catalogProducts = useSelector(
    (state) => state.product?.catalogProducts ?? [],
  );
  ...
  const { error: paymentErr, isLoading, Razorpay } = useRazorpay();

  const handlePayment = async (amount, currency, cartId) => {
    const { order } = await handleCreateOrder(amount, currency, cartId);
    console.log(order);
    console.log("user in payment", user);
    const options = {
      key: "rzp_test_SiPsfbm67hfpUR",
      amount: order.amount,
      currency: order.currency,
      name: "BaddieThrift",
      description: "Be a Baddie choose thrifting.",
      order_id: order.id,
      handler: async (response) => {
        if (response) {
          const succeed = await handleVerifyOrder(
            ...
            <aside className="w-full lg:w-[400px] flex flex-col gap-6">
              ...
              <div className="border-2 border-black bg-white p-8 sticky top-24">
                ...
                <div className="flex flex-col gap-4 mb-8">
                  <div
                    className="flex justify-between font-black text-xs uppercase tracking-widest"
                    style={{ fontFamily: "'Space Grotesk', sans-serif" }}
                  >
                  ...
                  {promoApplied && (
                    <div
                      className="flex justify-between font-black text-xs uppercase tracking-widest text-[#506600]"
                      style={{ fontFamily: "'Space Grotesk', sans-serif" }}
                    >
                ...
                <div className="flex flex-col gap-4">
                  <button
                    onClick={() => {
                      handlePayment(total, cartCurrency, cartId);
                    }}
              ...
              <div className="border-2 border-black bg-black p-8 text-white">
                ...
                <div className="flex flex-col gap-3">
                  {["SHIPPING & RETURNS", "SIZE GUIDE", "CONTACT SUPPORT"].map(
                    (link) => (
                      <a
                        key={link}
                        href="#"
              ...
              {suggestedProducts.map((product) => {
                ...
                return (
                  <button
                    ...
                    onClick={() =>
                      navigate(
                        `/product/${product._id}/${product?.variants[0]._id}`,
                      )
                    }
